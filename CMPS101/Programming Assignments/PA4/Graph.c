...
// insertSorted() inserts v into list u in its numerical position
...
void freeGraph(Graph* pG) {
	// freeGraph() frees all of the Vertex. Vertex adjacency list
	for (int i = 1; i <=getOrder(*pG); i++) {
		freeList(&(*pG)->neighbors[i]);
	}
	// here, the code frees malloc'd memory for 
	free((*pG)->neighbors);
	free((*pG)->color);
	free((*pG)->distance);
	free((*pG)->parent);
	// NULLify(?), (or makes to NULL) the pointers
	(*pG)->neighbors = NULL;
	(*pG)->color = NULL;
	(*pG)->parent = NULL;
	(*pG)->distance = NULL;
...
// getSize() returns the number of edges on Graph G
int getSize(Graph G) {
	if (G != NULL) {
		return G->size;
	} else {
...
// getSource() returns the label of most recent vertex called by BFS()
// however, if BFS has not been called, then code allows to return to NIL
...
// getParent() returns the parent vertex of vertex u in the Breadth-First Tree
// created by BFS()
...
// this code returns the distance from the most recent BFS source to vertex u.
...
void BFS(Graph G, int s) {
	...
	while(length(Q) != 0) {
		...
		for (moveFront(X, 0); index(X) != -1; moveNext(X)) {
			int y = get(X);
			if (G->color[y] == WHITE) {
				G->color[y] = GRAY;
