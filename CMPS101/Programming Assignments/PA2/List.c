...
//Structure of the codes are following: 
...
// newNode()
// This returns reference to new Node object. Initializes next, prev and data fields.
// Private.
...
// freeNode()
// this code frees heap memory pointed to by *pN, sets *pN to NULL.
// Private.
...
// newList()
// This returns reference to a new empty List object
List newList(void) {
	List L;
...
// freeList()
// it frees all heap memory associated with List *pL, and sets *ppL to NULL
...
void moveFront(List L, int i) {
	...
	if (i == L->length-1) L->cursor = L->back;
		...
		// These codes find the shortest distance. 
		int min = d1;
		if (min > d2) min = d2;
		if (min > d3) min = d3;
		// Here, the code set up the traversal from the back of the list
		if (min == d1) {
			L->cursor = L->back;
...
// append()
// This codes insert data the the end of the list. 
...
void deleteFront(List L) {
	if (L == NULL) {
		...
		L->front = a;
		if (L->cursorIndex != -1) L->cursorIndex--;
		if (L->cursor == temp) moveFront(L, -1);
		freeNode(&temp);
	} L->length--;
}
