...
//This section creates a new empy list 
	List () {
		front 		= null;
		back 		= null;
		cursor 		= null;
...
//this returns the index of the cursor element in this list, or -1 if the cursor is undefined.
	int index () {
		return cursorIndex;
	}
//Returns front element in the list. 
//Pre: length()>0
	Object front () {
		if (length() != 0) {
			return front.data;
...
//Returns back element in the list. 
//Pre: int length()>0
	Object back () {
		if (length() != 0) {
			return back.data;
...
//Pre: length()>=0, index()>=0
	Object get () {
		if (length() == 0) {
			...
			throw new RuntimeException("Error: get():  cursor undefined.");
		} else {
			return cursor.data;
		}
	}
//this returns true if the list and L are the same integer sequence
...
//**If 0<=i<=length()-1, then this code moves the cursor to the element
// at index i; otherwise the cursor becomes undefined.
// this made the best traversal.
...
//this inserts data to the end of the list
